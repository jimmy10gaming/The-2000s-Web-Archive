...
})}this._readyCallbacks=null},onDOMReady:function(f){if(AC.isDomReady){f()}else{if(!this._readyCallbacks){var domReady=this._domReady.bind(this);
...
}}};Object.extend(Element,Element2.Methods);function documentHeight(){var a=document;
return Math.max(Math.max(a.body.scrollHeight,a.documentElement.scrollHeight),Math.max(a.body.offsetHeight,a.documentElement.offsetHeight),Math.max(a.body.clientHeight,a.documentElement.clientHeight))
}if(typeof(AC)=="undefined"){AC={}}if(typeof(AC.Tracking)=="undefined"){AC.Tracking={}
...
break}}return e};AC.Tracking.recordScrollCoverage=function(){var e=document.viewport.getScrollOffsets(),f=e[1],b=document.viewport.getHeight()+f,k=window.documentHeight(),h=Math.round(b/k*100),j=document.cookie.split(";"),g=null,d,c,a;
for(c=0;(a=j[c]);c++){if((d=a.match(/^\s*s_ppv=(.+)\s*$/))){g=d[1];break}}if(h>g){document.cookie="s_ppv="+h+"; domain=.apple.com; path=/"
}};Event.onDOMReady(function(){AC.Tracking.recordScrollCoverage();Event.observe(window,"scroll",function(a){AC.Tracking.recordScrollCoverage()
});Event.observe(window,"resize",function(a){AC.Tracking.recordScrollCoverage()
})});AC.Tracking.track=function(f,d,b){if(typeof(s_gi)=="undefined"||!s_gi){return
...
g++){if(h.name===d){a[a.length]=h}}}return a}}var gomez=gomez?gomez:{};gomez.h3=function(c,a){for(var b in a){c[b]=a[b]
}return c};gomez.h3(gomez,{b3:function(a){if(a<=0){return false}return Math.random()<=a&&a
...
a.x=location.protocol+"//"+b+"/mr/b.gif?";a.y=location.protocol+"//"+b+"/mr/a.gif?"
...
/*
     FILE ARCHIVED ON 13:49:04 Jul 01, 2009 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 15:32:36 Nov 22, 2025.
...
playback timings (ms):
  ...
  esindex: 0.014
  cdx.remote: 9.609
  LoadShardBlock: 386.104 (6)
  PetaboxLoader3.datanode: 378.87 (7)
  load_resource: 191.632
  PetaboxLoader3.resolve: 126.148
*/